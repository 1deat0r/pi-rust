...
| TUI-043 | native image/clipboard backends | PARTIAL | PARTIAL | OPEN | Fallback tests pass; Wave D adds 0.85.1 capability overrides (`PI_HYPERLINKS`/`PI_IMAGE_PROTOCOL`/`PI_TRUE_COLOR` + `set_capability_overrides`) with unit pins, and wires them into settings resolution. Native `.node` prebuilds stay platform-native (Rust uses OS clipboard programs); Wayland/X11/Termux runtime evidence remains. |
...
Phase 2.5 (pi-coding-agent gate) likewise does not touch TUI rows.
