...
### Session slice AK: empty prompt creates no session file (last updated 2026-09-23)
...
### Session slice AJ: docs checkpoint helper + post-commit always-push (last updated 2026-09-23)
...
`--skill` path reaches the provider payload (input-token usage
rises versus the same turn without it). No source change needed;
row note extended, statuses held. Gate: resources suite 10/10, fmt
clean, conversion 100.00% (166/166), parity register + dashboard
OK (upstream=d7296c0). No parity row promoted
(invalid/precedence breadth open). Metrics unchanged
(implementation 111/266, deterministic evidence 107/266, runtime
59/266, non-TUI overall 58/266, whole-product 58/318).
...
HEAD, unrelated to any slice): the nvidia id list (now 20 ids incl.
`deepseek-ai/deepseek-v4-pro-0813`, minus 3 removed) and the copilot
OAuth filter fixture (`gpt-4.1` retired → `gpt-5-mini`, same filter
path). Full catalog suite 14/14 + copilot 5/5 green. Gate: pi-ai
lib 483/483, pi-ai strict clippy clean, fmt clean, conversion
100.00% (166/166), parity dashboard OK (upstream=d7296c0). No
parity row promoted (test-only). Metrics unchanged (implementation
111/266, deterministic evidence 107/266, runtime 59/266, non-TUI
overall 58/266, whole-product 58/318).
...
google-generative-ai lane now wraps its initial request in the
shared retry policy (opt-in via maxRetries) — transient transport
errors and retryable 408/409/429/5xx statuses retry with backoff
(honoring retry-after up to the delay cap) instead of ending the
turn, mirroring the Vertex `send_google_request` loop. New
loopback pin (503→200 succeeds in 2 requests with maxRetries=1;
503 alone is terminal with maxRetries=0). TDD red first (plus a
server-await hang fixed via abort pattern). Gate: pi-ai lib
483/483, exhaustive 18/18, pi-ai strict clippy clean, fmt clean,
conversion 100.00% (166/166), parity dashboard OK
(upstream=d7296c0). No parity row promoted (provider-lane
deterministic slice; live Google retry timing unverifiable
offline). Metrics unchanged (implementation 111/266,
deterministic evidence 107/266, runtime 59/266, non-TUI overall
58/266, whole-product 58/318).
...
Vertex, and OpenCode thinking maps derive from models.dev effort
metadata instead of regex rules (Gemma 4 toggle fallback kept).
32 thinking models updated across the three data files using live
models.dev as the oracle (matches the mock metadata in the upstream
generation test); alias sources honored (flash-latest ← 3.5-flash,
flash-lite-latest ← 3.1-flash-lite). New pin mirrors the upstream
providers test (3.6-flash minimal, 3.8-flash/pro-preview
low/medium/high, opencode 3.8-flash, gemma-4-31b-it minimal/high).
TDD red first. Gate: pi-ai lib 474/474, catalog pin green, pi-ai
strict clippy clean, fmt clean, conversion 100.00% (166/166), parity
dashboard OK (upstream=d7296c0). The stale `moonshot_and_nvidia`
pin stays red on clean HEAD (unrelated). No parity row promoted
(data-correction slice; live vendor catalog unverifiable offline).
Metrics unchanged (implementation 111/266, deterministic evidence
107/266, runtime 59/266, non-TUI overall 58/266, whole-product
58/318).
...
GPT-5.4 mini removed from the OpenAI Codex catalog (unavailable to
ChatGPT accounts); the `openai` provider entries are untouched. New
`retired_codex_models_are_absent_upstream_9394` pin (both gone,
GPT-5.5 successor remains). Count pins updated 1353→1351 with
attribution. TDD red first. Gate: pi-ai lib 473/473, catalog pins
green, pi-ai strict clippy clean, fmt clean, conversion 100.00%
(166/166), parity dashboard OK (upstream=d7296c0). The stale
`moonshot_and_nvidia` pin stays red on clean HEAD (unrelated). No
parity row promoted (data-correction slice; live vendor catalog
unverifiable offline). Metrics unchanged (implementation 111/266,
deterministic evidence 107/266, runtime 59/266, non-TUI overall
58/266, whole-product 58/318).
...
uses `reasoning_effort` on the Mistral lane — its `prompt_mode`
parameter is ignored upstream, so reasoning would silently disable
without this arm. One-line addition to `uses_reasoning_effort` plus
`zai_glm_5_2_uses_effort_instead_of_prompt_mode_upstream_9375` pin
(effort high + no prompt_mode with thinking; neither without). TDD
red first. Gate: pi-ai lib 472/472, pi-ai strict clippy clean, fmt
clean, conversion 100.00% (166/166), parity dashboard OK
(upstream=d7296c0). No parity row promoted (provider-lane
deterministic slice; live Mistral GLM traffic unverifiable offline).
Metrics unchanged (implementation 111/266, deterministic evidence
107/266, runtime 59/266, non-TUI overall 58/266, whole-product 58/318).

## Current status (last updated 2026-09-16 — Wave C slice: Mistral merge)

Wave C slice (0.85.1, upstream #8387 fixed by 6c87d9a02): the
Mistral conversations streaming path now merges tool-call chunks by
`index ?? callId` instead of `{callId}:{index}`, so a follow-up chunk
with the same index but no id (and empty name) appends arguments to
the existing block rather than opening a second block. Rust keys are
`index:{n}` when an explicit index is present, `id:{callId}`
otherwise. New `merges_fragmented_tool_calls_by_index_upstream_8387`
pin (one merged block, original id/name, concatenated arguments).
TDD red first. Gate: pi-ai lib 471/471, pi-ai strict clippy clean,
fmt clean, conversion 100.00% (166/166), parity dashboard OK
(upstream=d7296c0). No parity row promoted (provider-lane
deterministic slice; live Mistral fragment streams unverifiable
offline). Metrics unchanged (implementation 111/266, deterministic
evidence 107/266, runtime 59/266, non-TUI overall 58/266,
whole-product 58/318).
...
`gpt-*` models route through the Responses API. The stale
`gpt-6-astra` entry under `openai-completions` in the vendored
`github-copilot.json` moved to `openai-responses` with Responses-style
compat (`supportsOpenAIGrammarTools`) and the full Copilot thinking map
(`off:null, minimal/low:low, medium/high/xhigh/max` verbatim, matching
the `gpt-5.6-*` siblings). New regression pin
`copilot_gpt_models_route_through_responses_api` in
`model_catalog_parity` (mirrors the upstream catalog-type test: every
`gpt-*` Copilot entry uses `openai-responses`, `gpt-6-astra` present).
Gate: new pin green, pi-ai lib 467/467, pi-ai strict clippy clean, fmt
clean, conversion 100.00% (166/166), parity dashboard OK
(upstream=d7296c0). The `moonshot_and_nvidia` catalog pin is red on
clean HEAD too (stale nvidia list, unrelated). No parity row promoted
(data-correction slice; live Copilot traffic unverifiable offline).
Metrics unchanged (implementation 111/266, deterministic evidence
107/266, runtime 59/266, non-TUI overall 58/266, whole-product 58/318).
...
walk-up-then-down was already unit-pinned in pi-ai. Row now
PASS/PASS/PASS; metrics move to deterministic evidence 104/266, runtime
56/266, non-TUI overall 55/266, whole-product 55/318.
Current dashboard metrics:
...
verified append; deleted-file races fail closed by construction (repo open
→ NotFound). Row now PASS/PASS/PASS; metrics move to implementation
108/266, deterministic evidence 103/266, runtime 55/266, non-TUI overall
54/266, whole-product 54/318.
Current dashboard metrics:
...
CLI-015 `--session-id` PASS (2026-09-06): one process test closes the
semantics — new-ID creation with header id, existing-ID reopen + append
without duplicates, unknown-ID warn-and-create (exact upstream text),
invalid-ID fail-closed. Row now PASS/PASS/PASS; metrics move to
implementation 107/266, deterministic evidence 102/266, runtime 54/266,
non-TUI overall 53/266, whole-product 53/318.
Current dashboard metrics:
...
CLI-016 `--fork` PASS (2026-09-06): one process test closes the matrix —
fork by bare session id with parent linkage, source untouched by child
writes, unknown-target fail-closed diagnostic. Row now PASS/PASS/PASS;
metrics move to deterministic evidence 101/266, runtime 53/266, non-TUI
overall 52/266, whole-product 52/318.
Current dashboard metrics:
...
CLI-014 `--session` PASS (2026-09-06): one process test closes the matrix —
missing-parent explicit paths fail closed without creating the directory;
read-only targets fail append (root-safe self-skip). Selector resolution
units and exact-id reopen were already pinned. Row now PASS/PASS/PASS;
metrics move to deterministic evidence 100/266, runtime 52/266, non-TUI
overall 51/266, whole-product 51/318.
Current dashboard metrics:
...
retired unknown-model hard error; binary does the upstream-correct fallback)
and was rewritten to pin warning + raw mode + restoration, stale fixture
removed (red verified on clean HEAD first). CLI-038 gained a second-geometry
resize case. Metrics move to deterministic evidence 98/266, non-TUI overall
50/266, whole-product 50/318.
Current dashboard metrics:
...
continue in this directory` (upstream discovery skips unreadable headers),
and byte-restore recovery completes with verified append. No source change.
Row now PASS/PASS/PASS; metrics move to deterministic evidence 96/266,
non-TUI overall 48/266, whole-product 48/318.
Current dashboard metrics:
