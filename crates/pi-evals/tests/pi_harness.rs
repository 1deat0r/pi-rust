...
fn prefers_an_explicit_harness_model_over_environment_defaults() {
    let selection = resolve_model_selection(
        Some(&ModelSelection { provider: "anthropic".into(), id: "claude-opus-4-6".into() }),
        Some(("openai-codex", "gpt-5.6-sol")),
    )
    .unwrap();
    assert_eq!(selection, ModelSelection { provider: "anthropic".into(), id: "claude-opus-4-6".into() });
}

#[test]
fn uses_trimmed_environment_defaults_when_the_harness_has_no_explicit_model() {
    let selection = resolve_model_selection(None, Some((" openai-codex ", " gpt-5.6-sol "))).unwrap();
    assert_eq!(selection, ModelSelection { provider: "openai-codex".into(), id: "gpt-5.6-sol".into() });
}
...
fn rejects_an_incomplete_model_selection() {
    for case in [
        ...
        let explicit = case.0.map(|p| ModelSelection { provider: p.to_string(), id: "x".into() });
        let env = case.1;
