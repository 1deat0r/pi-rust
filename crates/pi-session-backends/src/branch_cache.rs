...
use crate::storage::branch_entries::{
    copy_branch_entries_through_seq, delete_branch_entries, insert_branch_entries_for_path, insert_branch_entry,
...
pub fn rebuild_branch_cache(db: &Connection, session_id: &str) -> Result<(), SessionError> {
    ...
    delete_branch_cache(db, session_id)
        .map_err(|error| SessionError::new(SessionErrorKind::Storage, format!("Failed to clear branch cache: {error}")))?;
    for tip in tips {
...
pub fn build_cached_branch(db: &Connection, session_id: &str, leaf_id: &str) -> Result<(), SessionError> {
    db.execute_batch("SAVEPOINT build_branch_cache")
        .map_err(|error| SessionError::new(SessionErrorKind::Storage, format!("Failed to begin branch cache build: {error}")))?;
    let result = (|| -> Result<(), SessionError> {
        ...
        insert_branch_tip(db, session_id, leaf_id, &branch_id)
            .map_err(|error| SessionError::new(SessionErrorKind::Storage, format!("Failed to insert branch tip: {error}")))?;
        Ok(())
    })();
    match result {
        Ok(()) => {
            db.execute_batch("RELEASE SAVEPOINT build_branch_cache")
                .map_err(|error| SessionError::new(SessionErrorKind::Storage, format!("Failed to release branch cache savepoint: {error}")))?;
            Ok(())
        }
        Err(error) => {
...
fn extend_branch(
    ...
    insert_branch_entry(db, session_id, branch_id, entry_id, entry_seq, entry_type, custom_type)
        .map_err(|error| SessionError::new(SessionErrorKind::Storage, format!("Failed to extend branch: {error}")))?;
    if !update_branch_tip(db, session_id, branch_id, parent_id, entry_id)
        .map_err(|error| SessionError::new(SessionErrorKind::Storage, format!("Failed to update branch tip: {error}")))? {
...
pub fn append_entry_to_branch_cache(
    ...
    let tip_branch_id = read_branch_tip_branch_id(db, session_id, parent_id)
        .map_err(|error| SessionError::new(SessionErrorKind::Storage, format!("Failed to read branch tip: {error}")))?;
    if let Some(tip_branch_id) = tip_branch_id {
        return extend_branch(db, session_id, &tip_branch_id, parent_id, entry_id, entry_seq, entry_type, custom_type);
    }

    let source = read_branch_containing_entry(db, session_id, parent_id)
        .map_err(|error| SessionError::new(SessionErrorKind::Storage, format!("Failed to read branch membership: {error}")))?;
    ...
    copy_branch_entries_through_seq(db, session_id, &branch_id, &source.branch_id, source.leaf_seq)
        .map_err(|error| SessionError::new(SessionErrorKind::Storage, format!("Failed to copy branch entries: {error}")))?;
    insert_branch_entry(db, session_id, &branch_id, entry_id, entry_seq, entry_type, custom_type)
        .map_err(|error| SessionError::new(SessionErrorKind::Storage, format!("Failed to insert branch entry: {error}")))?;
    insert_branch_tip(db, session_id, entry_id, &branch_id)
        .map_err(|error| SessionError::new(SessionErrorKind::Storage, format!("Failed to insert branch tip: {error}")))?;
