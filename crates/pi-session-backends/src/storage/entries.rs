    ...
    match entry {
        ...
        Entry::ActiveTools {
            ...
            obj.insert(
                ...
                serde_json::json!(active_tool_names),
            );
        }
        Entry::Compaction {
            summary,
...
pub fn read_entry_rows(
    db: &Connection,
    session_id: &str,
    options: ReadEntryRowsOptions<'_>,
    ...
    query = query.inline(&SqlQuery::new(if oldest_first {
        " ORDER BY seq ASC"
    } else {
        " ORDER BY seq DESC"
    }));
...
pub fn id_exists_in_entries(db: &Connection, session_id: &str, id: &str) -> rusqlite::Result<bool> {
    Ok(
        SqlQuery::new("SELECT 1 AS found FROM entries WHERE session_id = ? AND id = ? LIMIT 1")
            .bind(session_id)
            .bind(id)
            .get_row(db, |_row| Ok(()))?
            .is_some(),
...
pub fn delete_entry_rows(db: &Connection, session_id: &str) -> rusqlite::Result<()> {
    SqlQuery::new("DELETE FROM entries WHERE session_id = ?")
        .bind(session_id)
        .run(db)?;
    Ok(())
}
