...
pub fn read_stats(db: &Connection, session_id: &str) -> Result<SessionStats, SessionError> {
    ...
    .map_err(|error| SessionError::new(SessionErrorKind::Storage, format!("Failed to read stats: {error}")))?;
    let row = row.ok_or_else(|| {
        SessionError::new(SessionErrorKind::Storage, format!("Missing stats row for session {session_id}"))
    })?;
    Ok(SessionStats {
        message_count: row.message_count as u64,
        cached_tokens: row.cached_tokens as u64,
        uncached_tokens: row.uncached_tokens as u64,
        total_tokens: row.total_tokens as u64,
        cost_total: row.cost_total,
    })
}

fn require_changes(result: rusqlite::Result<u64>, session_id: &str, what: &str) -> Result<(), SessionError> {
    match result {
        Ok(1) => Ok(()),
...
pub fn increment_message_count(db: &Connection, session_id: &str) -> Result<(), SessionError> {
    let result = SqlQuery::new("UPDATE session_stats SET message_count = message_count + 1 WHERE session_id = ?")
        .bind(session_id)
        .run(db)
        .map(|r| r.changes);
...
pub fn add_usage_to_stats(db: &Connection, session_id: &str, usage: &Usage) -> Result<(), SessionError> {
...
pub fn delete_stats(db: &Connection, session_id: &str) -> rusqlite::Result<()> {
    SqlQuery::new("DELETE FROM session_stats WHERE session_id = ?").bind(session_id).run(db)?;
    Ok(())
}
