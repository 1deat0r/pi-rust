...
pub fn read_branch_tip_branch_id(
    ...
    let tip = SqlQuery::new("SELECT branch_id FROM branch_tips WHERE session_id = ? AND tip_id = ?")
        .bind(session_id)
        .bind(tip_id)
...
pub fn delete_branch_tips(db: &Connection, session_id: &str) -> rusqlite::Result<()> {
    SqlQuery::new("DELETE FROM branch_tips WHERE session_id = ?").bind(session_id).run(db)?;
    Ok(())
}
