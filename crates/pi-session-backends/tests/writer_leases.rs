...
async fn shares_one_write_queue_across_repeated_opens_in_one_repository() {
    ...
    let mut session = repo
        .create(&SqliteSessionCreateOptions { id: Some("session".into()), cwd: root.to_string_lossy().into_owned(), ..Default::default() })
        .await
        .unwrap();
    let metadata = session.get_metadata().await.unwrap();
    let mut reopened = repo.open(&SqliteSessionMetadata::from_core(&metadata)).await.unwrap();
...
fn rejects_zero_ttl() {
    let root = create_temp_dir();
    let database_path = root.join("sessions.sqlite").to_string_lossy().into_owned();
    SqliteSessionRepository::new(database_path, Some(SqliteWriterLeaseOptions { ttl_ms: 0, heartbeat_interval_ms: 1 }));
}
...
fn rejects_non_positive_heartbeat() {
    let root = create_temp_dir();
    let database_path = root.join("sessions.sqlite").to_string_lossy().into_owned();
    SqliteSessionRepository::new(database_path, Some(SqliteWriterLeaseOptions { ttl_ms: 100, heartbeat_interval_ms: 100 }));
}
...
async fn lists_complete_metadata_without_acquiring_active_sessions_writer_leases() {
    ...
    let (first_metadata, second_metadata) = tokio::join!(first.get_sqlite_metadata(), second.get_sqlite_metadata());
    ...
    assert_eq!(first_metadata.metadata, Some(serde_json::json!({ "profile": "reviewer" })));
    ...
    let leases_before = SqlQuery::new("SELECT session_id, owner_id, fence, expires_at_ms FROM writer_leases ORDER BY session_id")
    ...
    let mut listed = reader_repo
        .list(&SqliteSessionListOptions { cwd: Some(root.to_string_lossy().into_owned()) })
        .await
        .unwrap();
    ...
    let leases_after = SqlQuery::new("SELECT session_id, owner_id, fence, expires_at_ms FROM writer_leases ORDER BY session_id")
    ...
    assert_eq!(err.kind, pi_agent::session::types::SessionErrorKind::Storage);
    assert!(err.message.contains("already has an active writer"), "got: {err}");
}
...
async fn rejects_a_second_writer_until_the_first_session_releases_its_claim() {
    ...
    let err = second_repo.open(&SqliteSessionMetadata::from_core(&metadata)).await.unwrap_err();
    assert!(err.message.contains("already has an active writer"), "got: {err}");

    first_repo.close().await;

    let mut second = second_repo.open(&SqliteSessionMetadata::from_core(&metadata)).await.unwrap();
    let id = second.append_message(user_message("new owner")).await.unwrap();
    assert!(!id.is_empty());
}
...
async fn fences_a_stale_owner_after_an_expired_lease_is_acquired_by_another_writer() {
    let root = create_temp_dir();
    let database_path = root.join("sessions.sqlite").to_string_lossy().into_owned();
    let lease = SqliteWriterLeaseOptions { ttl_ms: 120_000, heartbeat_interval_ms: 60_000 };
    ...
    let err = first.append_message(user_message("stale owner")).await.unwrap_err();
    assert_eq!(err.kind, pi_agent::session::types::SessionErrorKind::Storage);
    assert!(err.message.contains("writer lease was lost"), "got: {err}");
    assert!(second.find_entries(&Default::default()).await.unwrap().is_empty());

    let inspection = rusqlite::Connection::open(&database_path).unwrap();
    let current_lease = SqlQuery::new("SELECT owner_id, fence FROM writer_leases WHERE session_id = ?")
        .bind(metadata.id.as_str())
        .get_row(&inspection, |row| Ok((row.get::<_, String>(0)?, row.get::<_, i64>(1)?)))
        .unwrap();
    ...
    let lease_after = SqlQuery::new("SELECT owner_id, fence FROM writer_leases WHERE session_id = ?")
        .bind(metadata.id.as_str())
        .get_row(&after_stale_close, |row| Ok((row.get::<_, String>(0)?, row.get::<_, i64>(1)?)))
        .unwrap();
    assert_eq!(lease_after.as_ref().unwrap().1, 2);

    let id = second.append_message(user_message("current owner")).await.unwrap();
    assert!(!id.is_empty());
}
...
async fn serializes_lease_checked_writes_for_sessions_sharing_one_database_connection() {
    ...
    let mut first = repo
        ...
        .unwrap();
    let mut second = repo
        .create(&SqliteSessionCreateOptions { id: Some("session-2".into()), cwd: root.to_string_lossy().into_owned(), ..Default::default() })
        .await
        .unwrap();
...
async fn renews_an_idle_writer_lease_with_a_heartbeat() {
    let root = create_temp_dir();
    let database_path = root.join("sessions.sqlite").to_string_lossy().into_owned();
    let lease = SqliteWriterLeaseOptions { ttl_ms: 500, heartbeat_interval_ms: 100 };
    ...
    assert!(after > initial, "expected expiry to renew: initial {initial}, after {after}");
