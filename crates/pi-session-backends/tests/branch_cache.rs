...
async fn collects_complete_root_paths_for_branches_created_after_compaction() {
    ...
    let compaction_id = append_sqlite_compaction(&mut session, "summary", 100, None, None, vec![]).await.unwrap();
    session.append_message(assistant_message("first child")).await.unwrap();
    move_sqlite_main_lane(&mut session, Some(&compaction_id), None).await.unwrap();
    let branched_id = session.append_message(assistant_message("branched child")).await.unwrap();

    let db = rusqlite::Connection::open(&database_path).unwrap();
    let row = SqlQuery::new("SELECT branch_id FROM branch_entries WHERE session_id = ? AND entry_id = ?")
        .bind("session-1")
        .bind(&branched_id)
...
async fn reads_only_the_compacted_branch_window_from_the_complete_cache() {
    let root = create_temp_dir();
    let database_path = root.join("sessions.sqlite");
    let repo = SqliteSessionRepository::new(database_path.to_string_lossy().into_owned(), Some(SqliteWriterLeaseOptions { ttl_ms: 5000, heartbeat_interval_ms: 1000 }));
    let mut session = repo
        .create(&SqliteSessionCreateOptions { id: Some("session-1".into()), cwd: root.to_string_lossy().into_owned(), ..Default::default() })
        .await
        .unwrap();
    ...
    let compaction_id = append_sqlite_compaction(&mut session, "summary", 100, None, None, vec![]).await.unwrap();
    let leaf_id = session.append_message(assistant_message("new")).await.unwrap();
    ...
    assert_eq!(
        branch.iter().map(|e| e.id().to_string()).collect::<Vec<_>>(),
...
async fn preserves_nested_compaction_boundaries_when_reading_the_cache() {
    ...
    append_sqlite_compaction(&mut session, "first summary", 100, None, None, vec![]).await.unwrap();
    session.append_message(user_message("middle")).await.unwrap();
    let second_compaction_id = append_sqlite_compaction(&mut session, "second summary", 200, None, None, vec![]).await.unwrap();
    let leaf_id = session.append_message(assistant_message("new")).await.unwrap();
...
async fn rejects_reads_and_writes_without_repairing_a_missing_private_branch_cache() {
    ...
    let mut session = repo
        ...
        .unwrap();
    session.append_message(user_message("root")).await.unwrap();
    session.append_message(assistant_message("child")).await.unwrap();

    {
        let db = rusqlite::Connection::open(&database_path).unwrap();
        db.execute_batch("DELETE FROM branch_tips WHERE session_id = 'session-1'").unwrap();
        db.execute_batch("DELETE FROM branch_entries WHERE session_id = 'session-1'").unwrap();
    }

    let err = get_sqlite_branch(&session).await.unwrap_err();
    assert_eq!(err.kind, SessionErrorKind::InvalidEntry, "got: {err}");
    let err = session.append_message(assistant_message("later")).await.unwrap_err();
    assert_eq!(err.kind, SessionErrorKind::InvalidEntry);
    assert!(err.message.contains("has no branch containing parent entry"), "got: {err}");
...
async fn repairs_the_private_branch_cache_explicitly() {
    ...
    let branch = get_sqlite_branch(&session).await.unwrap();
    assert_eq!(branch.iter().map(|e| e.id().to_string()).collect::<Vec<_>>(), vec![root_id, child_id]);
}
...
async fn fails_when_forking_from_a_source_with_a_missing_branch_cache() {
    ...
    let child_id = source.append_message(assistant_message("child")).await.unwrap();

    {
        let db = rusqlite::Connection::open(&database_path).unwrap();
        db.execute_batch("DELETE FROM branch_tips WHERE session_id = 'source'").unwrap();
        db.execute_batch("DELETE FROM branch_entries WHERE session_id = 'source'").unwrap();
    }
...
async fn fails_when_the_private_branch_cache_is_stale() {
    ...
    let mut session = repo
        ...
        .unwrap();
    let root_id = session.append_message(user_message("root")).await.unwrap();
    let stale_id = session.append_message(assistant_message("stale")).await.unwrap();
