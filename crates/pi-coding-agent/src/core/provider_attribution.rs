...
fn get_default_attribution_headers(
    ...
    if !settings.get_enable_install_telemetry() {
        return None;
    }
...
pub fn merge_provider_attribution_headers(
    ...
    if let Some(headers) = get_default_attribution_headers(model, settings) {
        for (name, value) in headers {
            merged.insert(name, Some(value));
        }
    }
...
mod tests {
    ...
    fn telemetry_disabled_yields_no_default_headers() {
        ...
        let merged = merge_provider_attribution_headers(&m, &settings, None, &[]);
    ...
    fn openrouter_headers_when_enabled() {
        ...
        let m = model("openrouter", "https://openrouter.ai/api/v1");
        let merged = merge_provider_attribution_headers(&m, &settings, None, &[]).unwrap();
    ...
    fn session_headers_only_for_opencode() {
        ...
        let merged = merge_provider_attribution_headers(&opencode, &settings, Some("sess-1"), &[]).unwrap();
        ...
        assert!(merge_provider_attribution_headers(&generic, &settings, Some("sess-1"), &[]).is_none());

        // No session id → no session headers.
        assert!(merge_provider_attribution_headers(&opencode, &settings, None, &[]).is_none());
    }
    ...
    fn later_source_headers_win() {
        ...
        let merged = merge_provider_attribution_headers(&m, &settings, None, &[Some(&override_headers)]).unwrap();
