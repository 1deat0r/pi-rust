...
mod markdown_tests {
    ...
    fn md(text: &str, width: usize) -> Vec<String> {
        ...
        m.render(width).into_iter().map(|l| strip(&l).trim_end().to_string()).collect()
    }
    ...
    fn preserves_source_list_markers_when_configured() {
        let opts = MarkdownOptions { preserve_ordered_list_markers: true, ..Default::default() };
        let lines = md_opts("  4. forth\n  3. third\n\n10) ten\n7) seven\n\n+ plus\n* star\n- minus\n+", 80, opts);
        assert_eq!(lines, vec![
    ...
    fn renders_blank_lines_between_loose_list_items() {
        ...
        assert_eq!(lines, vec![
            ...
            "2. Duis aute irure dolor.", "", "   Excepteur sint occaecat cupidatat.", "",
            "3. Beep boop",
        ]);
    }
    ...
    fn maintains_numbering_when_code_blocks_are_not_indented() {
        ...
        let numbered: Vec<&String> = lines.iter().filter(|l| l.trim_start().starts_with(|c: char| c.is_ascii_digit())).collect();
    ...
    fn indents_wrapped_nested_list_lines() {
        ...
        assert_eq!(lines, vec!["- parent", "    - alpha beta gamma", "      delta epsilon"]);
    }
    ...
    fn renders_code_inside_list_item() {
        ...
        assert_eq!(lines, vec!["- ```ts", "    alpha beta gamma", "  delta epsilon zeta", "  ```"]);
    }
    ...
    fn keeps_column_width_at_longest_word() {
        ...
        let data_line = lines.iter().find(|l| l.contains(longest_word)).expect("data row with longest word");
        let first_segment = data_line.split('│').nth(1).unwrap();
        let first_width = first_segment.trim().len();
        assert!(first_width >= longest_word.len(), "col width {first_width} < {}", longest_word.len());
    }
    ...
    fn wraps_table_cells_when_table_exceeds_available_width() {
        ...
        for line in &lines {
            assert!(line.chars().count() <= 50, "Line exceeds width 50: {line:?}");
        }
