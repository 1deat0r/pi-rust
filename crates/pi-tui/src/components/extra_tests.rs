...
mod components_extra_tests {
    ...
    use crate::components::settings_list::{SettingItem, SettingsList, SettingsListOptions, plain_settings_theme};
    use crate::terminal_image::{get_gif_dimensions, get_png_dimensions, set_capabilities, TerminalCapabilities, ImageProtocol};
    ...
    fn alt_screen_search_finds_matches_and_merges_segments() {
        ...
        let rows: Vec<usize> = matches.iter().flat_map(|m| m.segments.iter().map(|s| s.row)).collect();
    ...
    fn image_fallback_without_capabilities() {
        ...
        let image = Image::new(
            ...
            "image/png",
            theme,
            ImageOptions { filename: Some("/tmp/pic.png".to_string()), ..Default::default() },
        );
    ...
    fn kitty_capability_renders_image_protocol() {
        set_capabilities(TerminalCapabilities { images: Some(ImageProtocol::Kitty), true_color: true, hyperlinks: true });
        let theme = ImageTheme { fallback_color: Box::new(|s| s.to_string()) };
        let image = Image::new(
            "SGVsbG8=",
            "image/png",
            theme,
            ImageOptions { max_width_cells: Some(40), ..Default::default() },
        );
        let lines = image.render(80);
        assert!(lines.first().map(|l| l.starts_with("\x1b_G")).unwrap_or(false));
    }

    #[test]
    fn settings_list_cycles_values() {
        let items = vec![SettingItem::new("theme", "Theme", "dark", vec!["dark".to_string(), "light".to_string()])];
        let mut list = SettingsList::new(items, 10, plain_settings_theme(), SettingsListOptions::default());
