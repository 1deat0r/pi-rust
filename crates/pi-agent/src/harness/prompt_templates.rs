...
/// Parse YAML frontmatter from markdown content (upstream `parseFrontmatter`).
/// Returns `(frontmatter, body)`; `None` when YAML parsing fails. Content
/// without a leading `---` fence (or without a closing fence) returns an empty
