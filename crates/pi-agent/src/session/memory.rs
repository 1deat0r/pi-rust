...
impl InMemorySessionStorage {
    ...
    pub fn append_entry(&mut self, entry: EntryNoStats, lane: &str) -> Result<Entry, SessionError> {
        ...
        let entry = super::types::Entry::from_provisioned(
            entry,
            parent_id,
            self.state.next_sequence(),
            now_ms(),
        );
        self.state.apply_mutation(&Mutation::Entry {
    ...
    pub fn append_record(&mut self, new_record: NewRecord) -> Result<LaneRecord, SessionError> {
        ...
        let current_open_operation_id = self
            .state
            .open_operation_ids(new_record.lane())
            .first()
            .cloned();
        ...
        self.state.apply_mutation(&Mutation::Record {
            record: record.clone(),
        })?;
        Ok(record)
    }
    ...
    pub fn set_name(&mut self, name: Option<&str>) -> Result<(), SessionError> {
        self.state
            .apply_mutation(&Mutation::Fact(super::types::Fact::Name {
                seq: self.state.next_sequence(),
    ...
    pub fn set_label(&mut self, id: &str, label: Option<&str>) -> Result<(), SessionError> {
        ...
        self.state
            .apply_mutation(&Mutation::Fact(super::types::Fact::Label {
                seq: self.state.next_sequence(),
...
impl InMemorySessionRepo {
    ...
    pub fn create(
        ...
        let id = id
            .map(|s| s.to_string())
            .unwrap_or_else(super::session::new_id);
    ...
    fn require_storage(
        &self,
        id: &str,
    ) -> Result<&Arc<Mutex<InMemorySessionStorage>>, SessionError> {
