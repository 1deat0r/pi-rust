...
use pi_ai::types::{ContentBlock, Cost, Message, SimpleStreamOptions, StopReason, Usage, UserContent};
...
fn scripted_models(responses: Vec<pi_ai::types::AssistantMessage>) -> (SimpleModels, Arc<Mutex<Vec<(String, SimpleStreamOptions)>>>) {
    ...
    let models = SimpleModels::new(move |_model, context, options| {
        ...
        Box::pin(async move {
            ...
            queue.lock().unwrap().pop_front().unwrap_or_else(|| {
                faux_assistant_message(vec![], FauxAssistantOptions {
                    stop_reason: Some(StopReason::Error),
                    error_message: Some("No scripted completeSimple response queued".to_string()),
                })
            })
        })
    });
...
fn calculates_total_context_tokens_from_usage() {
    ...
    // totalTokens is 0.
    assert_eq!(pi_agent::harness::compaction::calculate_context_tokens(&usage), 1800);
}

#[test]
fn checks_compaction_threshold() {
    let settings = CompactionSettings { enabled: true, reserve_tokens: 10000, keep_recent_tokens: 20000 };
    assert!(should_compact(95_000, 100_000, &settings));
    assert!(!should_compact(89_000, 100_000, &settings));
    assert!(!should_compact(95_000, 100_000, &CompactionSettings { enabled: false, ..settings }));
}
...
fn builds_session_context_with_compaction_entry() {
    ...
    let roles: Vec<String> = loaded.messages.iter().map(|m| m.role().to_string()).collect();
    assert_eq!(roles, vec!["compactionSummary", "user", "assistant", "user", "assistant"]);
}

#[test]
fn prepares_compaction_using_latest_summary_as_previous() {
    let u1 = message_entry(user_message("user msg 1"), "u1", None, 1);
    let a1 = message_entry(assistant_message("assistant msg 1", None), "a1", Some("u1"), 2);
    ...
    let preparation = prepare_compaction(&path_entries, &DEFAULT_COMPACTION_SETTINGS).unwrap().unwrap();
...
fn carries_previous_compaction_retained_tail_into_next_preparation() {
    ...
    let compaction = compaction_entry("previous summary", "c1", None, 1, vec![retained_user.clone(), retained_assistant.clone()], None);
    let user = message_entry(user_message("new user"), "u1", Some("c1"), 2);
    let assistant = message_entry(assistant_message("new assistant", None), "a1", Some("u1"), 3);

    let settings = CompactionSettings { enabled: true, reserve_tokens: 100, keep_recent_tokens: 1 };
    let preparation = prepare_compaction(&[compaction, user, assistant], &settings).unwrap().unwrap();
    assert_eq!(preparation.previous_summary.as_deref(), Some("previous summary"));
    ...
    assert_eq!(
        collected,
        vec![retained_user, retained_assistant, user_message("new user"), assistant_message("new assistant", None)]
    );
}
...
fn prepares_split_turn_compaction_with_prior_file_operation_details() {
    ...
    let a1_message = {
        ...
        *am.content_mut() = vec![ContentBlock::tool_call("tool-1", "write", serde_json::json!({"path": "written.ts"}))];
    ...
    let compaction1 = compaction_entry(
        ...
        3,
        vec![],
        Some(serde_json::json!({"readFiles": ["old-read.ts"], "modifiedFiles": ["old-edit.ts", "written.ts"]})),
    );
    let u2 = message_entry(user_message("large turn but small"), "u2", Some("c1"), 4);
    let a2 = message_entry(assistant_message("large assistant message", None), "a2", Some("u2"), 5);
    let settings = CompactionSettings { enabled: true, reserve_tokens: 100, keep_recent_tokens: 1 };
    let preparation = prepare_compaction(&[u1, a1, compaction1, u2, a2], &settings).unwrap().unwrap();
...
async fn passes_reasoning_only_for_reasoning_models_with_thinking_enabled() {
    ...
    let (models, calls) = scripted_models(vec![
        faux_assistant_message(vec![ContentBlock::text("## Goal\nTest summary")], FauxAssistantOptions::default()),
    ]);
    let model = faux_model(true, 8192);
    generate_summary(&messages, &models, &model, 2000, None, None, None, Some("medium"), None, None).await.unwrap();
    let captured = calls.lock().unwrap().clone();
    assert_eq!(captured[0].1.reasoning, Some(pi_ai::types::ThinkingLevel::Medium));

    let (models, calls) = scripted_models(vec![
        faux_assistant_message(vec![ContentBlock::text("## Goal\nTest summary")], FauxAssistantOptions::default()),
    ]);
    generate_summary(&messages, &models, &model, 2000, None, None, None, Some("off"), None, None).await.unwrap();
    let captured = calls.lock().unwrap().clone();
    ...
    generate_summary(&messages, &models, &non_reasoning, 2000, None, None, None, Some("medium"), None, None).await.unwrap();
    let captured = calls.lock().unwrap().clone();
    assert_eq!(captured[0].1.reasoning, None);
}
...
async fn includes_previous_summaries_and_custom_instructions_in_prompts() {
    ...
    let summary = generate_summary_with_usage(&messages, &models, &model, 2000, None, Some("focus"), Some("old summary"), None, None, None)
        .await
        .unwrap();
...
async fn returns_errors_for_failed_or_aborted_summary_generations() {
    ...
    let (models, _) = scripted_models(vec![faux_assistant_message(
        vec![],
        FauxAssistantOptions { stop_reason: Some(StopReason::Error), error_message: Some("boom".into()) },
    )]);
    let err = generate_summary(&messages, &models, &model, 2000, None, None, None, None, None, None).await.unwrap_err();
    assert_eq!(err.code, "summarization_failed");
    assert_eq!(err.message, "Summarization failed: boom");

    let (models, _) = scripted_models(vec![faux_assistant_message(
        vec![],
        FauxAssistantOptions { stop_reason: Some(StopReason::Aborted), error_message: Some("stopped".into()) },
    )]);
    let err = generate_summary(&messages, &models, &model, 2000, None, None, None, None, None, None).await.unwrap_err();
...
async fn clamps_max_tokens_and_isolates_requests() {
    ...
    let preparation = CompactionPreparation {
        ...
        settings: CompactionSettings { enabled: true, reserve_tokens: 500_000, keep_recent_tokens: 20_000 },
    };
    compact(&preparation, &models, &model, None, None, None, None, None).await.unwrap();
    let captured = calls.lock().unwrap().clone();
    let max_tokens: Vec<_> = captured.iter().map(|(_, o)| o.base.max_tokens).collect();
    assert_eq!(max_tokens, vec![Some(128_000), Some(128_000)]);
    let cache: Vec<_> = captured.iter().map(|(_, o)| o.base.cache_retention.clone()).collect();
    assert_eq!(cache, vec![Some("none".to_string()), Some("none".to_string())]);
    let session_ids: Vec<_> = captured.iter().map(|(_, o)| o.base.session_id.clone()).collect();
...
async fn combines_usage_for_split_turn_compaction() {
    ...
    let (models, _) = scripted_models(vec![
        {
            let mut m = faux_assistant_message(vec![ContentBlock::text("history summary")], FauxAssistantOptions::default());
            m.set_usage(history_usage);
            m
        },
        {
            let mut m = faux_assistant_message(vec![ContentBlock::text("turn prefix summary")], FauxAssistantOptions::default());
    ...
    let result = compact(&preparation, &models, &model, None, None, None, None, None).await.unwrap();
...
async fn passes_reasoning_through_turn_prefix_summaries_when_enabled() {
    let messages = vec![user_message("Summarize this.")];
    let (models, calls) = scripted_models(vec![
        faux_assistant_message(vec![ContentBlock::text("## Original Request\nTest summary")], FauxAssistantOptions::default()),
    ]);
    let model = faux_model(true, 8192);
    let preparation = CompactionPreparation {
    ...
    compact(&preparation, &models, &model, None, None, Some("high"), None, None).await.unwrap();
    assert_eq!(calls.lock().unwrap()[0].1.reasoning, Some(pi_ai::types::ThinkingLevel::High));
}
...
async fn returns_compaction_result_with_file_details() {
    ...
    let a1_message = {
        let mut am = pi_ai::types::AssistantMessage::new();
        *am.content_mut() = vec![ContentBlock::tool_call("tool-1", "read", serde_json::json!({"path": "src/index.ts"}))];
        am.set_api_provider_model("anthropic-messages", "anthropic", "claude-sonnet-4-5");
    ...
    let a2 = message_entry(assistant_message("done", Some(mock_usage(4000, 500, 0, 0))), "a2", Some("u2"), 4);
    let path_entries = [u1, a1, u2, a2];
    let budget = CompactionSettings { enabled: true, reserve_tokens: 2000, keep_recent_tokens: 1 };
    let preparation = prepare_compaction(&path_entries, &budget).unwrap().unwrap();
    assert!(preparation.is_split_turn);
    let (models, _) = scripted_models(vec![
        faux_assistant_message(vec![ContentBlock::text("## Goal\nTest summary")], FauxAssistantOptions::default()),
        faux_assistant_message(vec![ContentBlock::text("## Original Request\nTest suffix")], FauxAssistantOptions::default()),
    ]);
    let model = faux_model(false, 8192);
    let result = compact(&preparation, &models, &model, None, None, None, None, None).await.unwrap();
...
async fn branch_summary_collects_abandoned_side_in_chronological_order() {
    ...
    let common_id = session.append_message(user_message("common")).await.unwrap();
    let abandoned_1 = session.append_message(user_message("abandoned 1")).await.unwrap();
    let abandoned_2 = session.append_message(user_message("abandoned 2")).await.unwrap();
    session.create_lane("target", Some(&common_id)).await.unwrap();
    let target_id = session.append_message_to_lane("target", user_message("target")).await.unwrap();

    let result = collect_entries_for_branch_summary(&session, Some(&abandoned_2), &target_id).await.unwrap();
    assert_eq!(result.common_ancestor_id.as_deref(), Some(common_id.as_str()));
...
async fn branch_summary_returns_no_entries_without_previous_leaf() {
    ...
    let mut session = Session::<MemoryFs>::from_in_memory(storage);
    let target_id = session.append_message(user_message("target")).await.unwrap();
    let result = collect_entries_for_branch_summary(&session, None, &target_id).await.unwrap();
...
async fn generate_branch_summary_empty_returns_no_content_to_summarize() {
    let model = faux_model(false, 8192);
    let (models, _) = scripted_models(vec![]);
    let result = generate_branch_summary(&[], &models, &model, &GenerateBranchSummaryOptions::default()).await.unwrap();
...
async fn generate_branch_summary_prepends_preamble_and_appends_file_ops() {
    let entries = vec![message_entry(
        {
            let mut am = pi_ai::types::AssistantMessage::new();
            *am.content_mut() = vec![ContentBlock::tool_call("t1", "write", serde_json::json!({"path": "new.ts"}))];
            am.set_api_provider_model("anthropic-messages", "anthropic", "claude-sonnet-4-5");
    ...
    let model = faux_model(false, 8192);
    let (models, _) = scripted_models(vec![
        faux_assistant_message(vec![ContentBlock::text("## Goal\nBranch work")], FauxAssistantOptions::default()),
    ]);
    let result = generate_branch_summary(&entries, &models, &model, &GenerateBranchSummaryOptions::default()).await.unwrap();
    assert!(result.summary.starts_with("The user explored a different conversation branch before returning here."));
...
fn prepare_branch_entries_respects_token_budget() {
    ...
    for i in 0..4 {
        ...
        entries.push(message_entry(user_message(&format!("message {i}")), &id, parent.as_deref(), i as u64 + 1));
        prev = Some(id);
    }
