...
fn assistant_message(text: &str) -> AgentMessage {
    AgentMessage::Core(Message::Assistant(pi_ai::types::AssistantMessage::Assistant {
        ...
        usage: Some(Usage {
            ...
            total_tokens: 0,
            cost: pi_ai::types::Cost { input: 0.0, output: 0.0, cache_read: 0.0, cache_write: 0.0, total: 0.0 },
        }),
...
fn deferred_assistant_message() -> AgentMessage {
    AgentMessage::Core(Message::Assistant(pi_ai::types::AssistantMessage::Assistant {
        ...
        deferred: Some(pi_ai::types::DeferredHandle {
            provider: "openai".into(), model_id: "gpt-5".into(), api: "openai-responses".into(),
            id: "response-1".into(), expires_at: None, poll_after_ms: None, data: None,
        }),
        error_message: None,
        raw_stop_reason: None,
        end_turn: None,
...
fn entry_message(id: &str, seq: u64, parent: Option<&str>, message: AgentMessage) -> Entry {
    Entry::Message {
        id: id.into(), seq, parent_id: parent.map(|s| s.to_string()), timestamp: seq,
        message, terminate: None,
    }
}
...
fn starts_at_latest_compaction_and_materializes_its_retained_tail() {
    let entries = vec![
        ...
        Entry::Compaction {
            id: "compact".into(), seq: 2, parent_id: Some("old".into()), timestamp: 2,
            summary: "summary".into(),
            retained_tail: vec![user_message("retained"), assistant_message("answer")],
            tokens_before: 100, details: None, usage: None,
        },
        Entry::ModelChange {
            id: "model".into(), seq: 3, parent_id: Some("compact".into()), timestamp: 3,
            provider: "openai".into(), model_id: "gpt-5".into(),
        },
        Entry::ThinkingLevel {
            id: "thinking".into(), seq: 4, parent_id: Some("model".into()), timestamp: 4,
    ...
    assert_eq!(roles(&context), vec!["compactionSummary", "user", "assistant", "user"]);
    assert_eq!(context.model, Some(("openai".to_string(), "gpt-5".to_string())));
...
fn applies_caller_transforms_after_the_compaction_boundary() {
    let entries = vec![
        ...
        Entry::Compaction {
            id: "compact".into(), seq: 2, parent_id: Some("old".into()), timestamp: 2,
            summary: "summary".into(), retained_tail: vec![], tokens_before: 100, details: None, usage: None,
        },
        Entry::BranchSummary {
            id: "branch".into(), seq: 3, parent_id: Some("compact".into()), timestamp: 3,
            from_id: "abandoned".into(), summary: "branch summary".into(), details: None, usage: None,
        },
    ...
    options.entry_transforms.push(Box::new(|entries: &[Entry]| {
        entries.iter().filter(|e| e.entry_type_str() != "compaction").cloned().collect()
    }));
...
fn projects_custom_entries_and_omits_deferred_assistant_handles() {
    let entries = vec![
        ...
        Entry::Custom {
            id: "custom".into(), seq: 3, parent_id: Some("deferred".into()), timestamp: 3,
            custom_type: "note".into(), data: Some(serde_json::json!("project me")),
        },
    ];
    ...
    options.entry_projectors.insert(
        ...
        Box::new(|entry: &Entry, _index: usize, _entries: &[Entry]| {
            let Entry::Custom { data, .. } = entry else { return None };
            let text = data.as_ref().and_then(|d| d.as_str()).map(|s| s.to_string()).unwrap_or_default();
            Some(vec![user_message(&format!("note: {text}"))])
        }),
    );
    let context = build_session_context(&entries, &options);
    assert_eq!(roles(&context), vec!["user", "user"]);
    let AgentMessage::Core(Message::User(u)) = &context.messages[1] else { panic!("expected user message") };
    assert_eq!(u.content(), &pi_ai::types::UserContentBody::Blocks(vec![ContentBlock::text("note: project me")]));
}
