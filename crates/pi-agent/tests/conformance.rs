...
pub trait ConformanceRepo: Send {
    async fn create(&mut self, options: RepoCreateOptions) -> Result<Session<MemoryFs>, SessionError>;
...
impl ConformanceRepo for InMemorySessionRepo {
    async fn create(&mut self, options: RepoCreateOptions) -> Result<Session<MemoryFs>, SessionError> {
        InMemorySessionRepo::create(self, options.id.as_deref(), options.parent_session_id.as_deref())
    }
    async fn open(&self, metadata: &SessionMetadata) -> Result<Session<MemoryFs>, SessionError> {
...
impl ConformanceRepo for JsonlSessionRepo<MemoryFs> {
    ...
    async fn list(&self) -> Result<Vec<SessionMetadata>, SessionError> {
        JsonlSessionRepo::list(self, None).await.map_err(|e| SessionError::new(SessionErrorKind::Storage, e.message))
    }
    async fn delete(&mut self, metadata: &SessionMetadata) -> Result<(), SessionError> {
...
mod commits_records_and_lane_moves_as_separate_mutations {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        ...
        assert_eq!(session.get_lanes().await, vec![pi_agent::session::types::LanePointer { lane: "main".into(), leaf_id: Some("root".into()) }]);
        session.move_lane("main", None).await.unwrap();
        assert_eq!(session.get_lanes().await, vec![pi_agent::session::types::LanePointer { lane: "main".into(), leaf_id: None }]);
        let log = session.get_log(&LogOptions::default()).await.unwrap();
        assert_eq!(log.len(), 3);
