...
use pi_agent::session::memory::InMemorySessionRepo;
use pi_agent::session::state::{
    BranchBounds, EntryCursor, EntryOrder, EntryQuery, ForkOptions, ForkPosition, LogOptions,
    RecordQuery,
};
...
pub trait ConformanceRepo: Send {
    ...
    ) -> Result<Session<MemoryFs>, SessionError>;
    async fn open(&self, metadata: &SessionMetadata) -> Result<Session<MemoryFs>, SessionError>;
...
#[async_trait]
impl ConformanceRepo for InMemorySessionRepo {
    async fn create(
        &mut self,
        options: RepoCreateOptions,
    ) -> Result<Session<MemoryFs>, SessionError> {
        InMemorySessionRepo::create(
            self,
            options.id.as_deref(),
    ...
    async fn open(&self, metadata: &SessionMetadata) -> Result<Session<MemoryFs>, SessionError> {
        InMemorySessionRepo::open(self, metadata)
    }
    async fn list(&self) -> Result<Vec<SessionMetadata>, SessionError> {
        Ok(InMemorySessionRepo::list(self))
    }
    async fn delete(&mut self, metadata: &SessionMetadata) -> Result<(), SessionError> {
        InMemorySessionRepo::delete(self, metadata);
        Ok(())
    }
    async fn fork(
        &mut self,
...
#[async_trait]
impl ConformanceRepo for JsonlSessionRepo<MemoryFs> {
    async fn create(
        &mut self,
        options: RepoCreateOptions,
    ) -> Result<Session<MemoryFs>, SessionError> {
        JsonlSessionRepo::create(
            self,
            CreateOptions {
                id: options.id,
    ...
    async fn open(&self, metadata: &SessionMetadata) -> Result<Session<MemoryFs>, SessionError> {
        JsonlSessionRepo::open(self, metadata).await
    }
    async fn list(&self) -> Result<Vec<SessionMetadata>, SessionError> {
        JsonlSessionRepo::list(self, None)
            .await
            .map_err(|e| SessionError::new(SessionErrorKind::Storage, e.message))
    }
    async fn delete(&mut self, metadata: &SessionMetadata) -> Result<(), SessionError> {
        JsonlSessionRepo::delete(self, metadata)
            .await
            .map_err(|e| SessionError::new(SessionErrorKind::Storage, e.message))
    }
    async fn fork(
...
fn mem_repo() -> Box<dyn ConformanceRepo> {
    Box::new(InMemorySessionRepo::new())
}

fn jsonl_repo() -> Box<dyn ConformanceRepo> {
    Box::new(JsonlSessionRepo::new(MemoryFs::new(), "/sessions"))
}

/// Runs one case body against both backends.
macro_rules! conformance_case {
    ($name:ident) => {
        #[tokio::test]
        async fn $name() {
            let mut mem = mem_repo();
            $name::__body(mem.as_mut()).await;
            let mut jsonl = jsonl_repo();
            $name::__body(jsonl.as_mut()).await;
        }
    };
}
...
mod assigns_parents_and_one_sequence_across_every_mutation {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        ...
        assert_eq!(
            (root.parent_id().map(|s| s.to_string()), root.seq()),
            (None, 1)
        );
        assert_eq!(
            (child.parent_id().map(|s| s.to_string()), child.seq()),
            (Some("root".to_string()), 3)
        );
        ...
        let kinds: Vec<(String, u64)> = log
            .iter()
            .map(|item| (item.kind().to_string(), item.seq()))
            .collect();
        assert_eq!(
            kinds,
            vec![
        ...
        assert_eq!(
            ...
            vec![
                pi_agent::session::types::LanePointer {
                    lane: "main".into(),
                    leaf_id: Some(child.id().to_string())
                },
                pi_agent::session::types::LanePointer {
                    lane: "thread".into(),
                    leaf_id: Some(child.id().to_string())
...
mod commits_records_and_lane_moves_as_separate_mutations {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        ...
        assert_eq!(
            session.get_lanes().await,
            vec![pi_agent::session::types::LanePointer {
                lane: "main".into(),
...
mod isolates_lanes_while_sharing_the_tree {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        ...
        let main_path = entry_ids(
            session
                .find_entries_on_branch(&q, Some("main-child"), &BranchBounds::default())
                .await
                .unwrap(),
        )
        .await;
        let thread_path = entry_ids(
            session
                .find_entries_on_branch(&q, Some("thread-child"), &BranchBounds::default())
                .await
                .unwrap(),
        )
        .await;
...
mod binds_lane_views_without_caching_leaves {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        ...
        assert_eq!(
            session.view("thread").get_leaf_id().await.unwrap(),
        ...
        let main_path = entry_ids(
            session
                .find_entries_on_branch(&q, None, &BranchBounds::default())
                .await
                .unwrap(),
        )
        .await;
        let thread_path = entry_ids(
            session
                .view("thread")
                .find_entries_on_branch(&q, &BranchBounds::default())
                .await
                .unwrap(),
        )
        .await;
        ...
        let empty_path = empty
            .find_entries_on_branch(&EntryQuery::default(), None, &BranchBounds::default())
            .await
            .unwrap();
...
mod appends_provisioned_entries_with_their_existing_ids {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        ...
        assert_eq!(
            (
                entry.id().to_string(),
                entry.parent_id().map(|s| s.to_string()),
                entry.seq()
            ),
            ("provisioned".to_string(), None, 1)
        );
        assert_eq!(
            session.get_leaf_id().await.unwrap(),
...
mod persists_tool_result_termination_decisions {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        let mut session = repo
            ...
            .unwrap();
        let entry = session
            .append_entry(tool_result_entry(), "main")
            .await
            .unwrap();
...
mod linearizes_concurrent_writes_across_two_lanes {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        ...
        assert_eq!(
            committed.iter().map(|e| e.seq()).collect::<Vec<_>>(),
            vec![3, 4, 5, 6]
        );
        assert_eq!(
            committed
                ...
                .collect::<Vec<_>>(),
            vec!["main-1", "thread-1", "main-2", "thread-2"]
        );
        let log_ids: Vec<String> = session
            ...
            .filter_map(|item| match item {
                pi_agent::session::types::LogItem::Entry(e)
                    if e.id().starts_with("main-") || e.id().starts_with("thread-") =>
                {
                    Some(e.id().to_string())
                }
                _ => None,
...
mod rejects_invalid_queries_before_empty_reads {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        ...
        let bad_limit = EntryQuery {
            limit: Some(0),
            ..Default::default()
        };
        assert_rejects(
            session.find_entries(&bad_limit).await.map(|_| ()),
            SessionErrorKind::InvalidQuery,
        );
        assert_rejects(
            session.find_entry(&bad_limit).await.map(|_| ()),
            SessionErrorKind::InvalidQuery,
        );
        assert_rejects(
            session
                .find_entries_on_branch(&bad_limit, None, &BranchBounds::default())
                .await
                .map(|_| ()),
            SessionErrorKind::InvalidQuery,
        );
        assert_rejects(
            session
                .find_entry_on_branch(&bad_limit, None, &BranchBounds::default())
                .await
                .map(|_| ()),
            SessionErrorKind::InvalidQuery,
        );
        assert_rejects(
            session
                .view("thread")
                .find_entries_on_branch(&bad_limit, &BranchBounds::default())
                .await
                .map(|_| ()),
            SessionErrorKind::InvalidQuery,
        );
        assert_rejects(
            session
                .view("thread")
                .find_entry_on_branch(&bad_limit, &BranchBounds::default())
                .await
                .map(|_| ()),
            SessionErrorKind::InvalidQuery,
        );
...
mod supports_bounded_filtered_and_cursor_based_queries {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        ...
        let r1 = entry_ids(
            session
                .find_entries_on_branch(&q1, Some("tail"), &b1)
                .await
                .unwrap(),
        )
        .await;
        ...
        let r2 = entry_ids(
            session
                .find_entries_on_branch(&q2, Some("tail"), &b2)
                .await
                .unwrap(),
        )
        .await;
        ...
        let r3 = entry_ids(
            session
                .find_entries_on_branch(&q3, Some("tail"), &b3)
                .await
                .unwrap(),
        )
        .await;
        ...
        let r4 = entry_ids(
            session
                .find_entries_on_branch(&q4, Some("tail"), &b4)
                .await
                .unwrap(),
        )
        .await;
...
mod keeps_latest_value_facts_and_computes_ledger_statistics_across_lanes {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        ...
        session
            .append_record(NewRecord::Usage {
                id: "deferred-usage".into(),
                lane: "main".into(),
                cause: "deferred_fetch".into(),
                run_id: Some("run".into()),
                entry_id: Some("deferred-result".into()),
                attempt: Some(1),
                stop_reason: Some("deferred".into()),
                tool_call_id: None,
                details: None,
                usage: zero_usage(),
            })
            .await
            .unwrap();
        // Provider corrections are first-class signed usage records. Their
        // negative token and cost deltas must be reflected in ledger stats.
        session
        ...
        let LaneRecord::Usage { stop_reason, .. } = deferred else {
            panic!("expected usage record")
        };
        assert_eq!(stop_reason.as_deref(), Some("deferred"));
        let LaneRecord::Usage { cause, usage, .. } = &usage_records[2] else {
            panic!("expected adjustment usage record")
        };
        assert_eq!(cause, "adjustment");
...
mod persists_queue_cancellation_without_consuming_its_target {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        ...
        let cancelled = session
            ...
            .await
            .unwrap();
        let LaneRecord::QueueCancelled { entry_id, .. } = &cancelled else {
        ...
        let cancellations = session
            .find_records(&RecordQuery {
                record_type: Some("queue_cancelled".into()),
                ..Default::default()
            })
            .await
            .unwrap();
        let LaneRecord::QueueCancelled { entry_id, .. } = &cancellations[0] else {
            panic!("expected queue_cancelled")
        };
...
mod filters_operation_starts_by_operation_kind {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        ...
        let runs = session
            ...
            .unwrap();
        assert_eq!(
            runs.iter().map(|r| r.id().to_string()).collect::<Vec<_>>(),
            vec!["run-old", "run-new"]
        );
        ...
        let navigations = session
            .find_records(&RecordQuery {
                record_type: Some("operation_started".into()),
                operation_kind: Some("navigation".into()),
                ..Default::default()
            })
            .await
            .unwrap();
...
mod returns_immutable_open_operation_records {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        ...
        let Some(first) = open.first() else {
            panic!("expected an open run operation")
        };
        let LaneRecord::OperationStarted { intent, .. } = first else {
            panic!("expected operation_started")
        };
        // Reads are clones: mutating the local copy must not affect storage.
        let mut intent_mut = intent.clone();
        if let pi_agent::session::types::OperationIntent::Run {
            original_prompt, ..
        } = &mut intent_mut
        {
...
mod returns_immutable_copies_from_reads {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        ...
        let entry_id = session
            .append_custom_entry("note", Some(data.clone()))
            .await
            .unwrap();
        ...
        let mut meta_clone = session.get_metadata().await;
        meta_clone.id = "changed".into();
        drop(meta_clone);

        let after = session.get_metadata().await;
        assert_eq!(after, metadata);
        let read_after = session.get_entry(&entry_id).await.unwrap();
        let Entry::Custom { data: d, .. } = &read_after else {
            panic!("expected custom")
        };
        assert_eq!(
            d.as_ref()
                .and_then(|v| v.get("nested").and_then(|n| n.get("value"))),
...
mod deletes_sessions_idempotently {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        ...
        assert_rejects(
            repo.open(&metadata).await.map(|_| ()),
            SessionErrorKind::NotFound,
        );
        repo.delete(&metadata).await.unwrap();
    }
...
mod forks_one_branch_with_selected_facts_and_no_records {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        ...
        assert_eq!(
            (
                fork_meta.id.as_str(),
                fork_meta.parent_session_id.as_deref()
            ),
...
mod forks_a_complete_tree_with_lanes_and_facts {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        ...
        assert_eq!(
            fork_ids,
            vec![root.clone(), main_child.clone(), thread_child.clone()]
        );
        assert_eq!(
            fork.get_lanes().await,
            vec![
        ...
        assert_eq!(
            fork.get_label(&thread_child).await,
            Some("thread-tip".to_string())
        );
        ...
        let lane_items: Vec<(u64, String, Option<String>)> = fork
            ...
            .iter()
            .filter_map(|item| match item {
                pi_agent::session::types::LogItem::Lane { seq, lane, leaf_id } => {
                    Some((*seq, lane.clone(), leaf_id.clone()))
                }
                _ => None,
            })
            .collect();
...
mod forks_before_an_entry_without_modifying_the_source {
    ...
    pub async fn __body(repo: &mut dyn ConformanceRepo) {
        ...
        assert_eq!(
            before_default.get_leaf_id().await.unwrap(),
            Some(root.clone())
        );
